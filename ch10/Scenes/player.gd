...
func animate():
	#Stationary player
	if velocity.x == 0 && velocity.y == 0 && hearts > 0 && isHit == false:
	...
	#Player moving right
	elif velocity.x >= 0 && velocity.y == 0 && hearts > 0:
		animations.play("run")
	...
	#Player is falling
	elif velocity.y > 0 and !is_wall_sliding && hearts > 0:
		animations.play("fall")
		
	#Player is jumping
	elif velocity.y < 0 and jump_count == 0 && hearts > 0:
